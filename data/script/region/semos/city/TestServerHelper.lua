...
local function createNPC()
	npc = npcHelper:createSpeakerNPC("Aida")
	...
	local giveAction = npcHelper:customAction(function(player, sentence, npc)
		...
		if not entityManager:isItem(target) then
		...
		local item = nil;
		if quantity > 1 then
			item = entityManager:getStackableItem(target)
			item:setQuantity(quantity)
		else
			item = entityManager:getItem(target)
	...
	local teleportAction = npcHelper:customAction(function(player, sentence, npc)
		local target = sentence:getTrimmedText():gsub("^%s*(.-)%s*$", "%1"):lower()
	...
	local meetAction = npcHelper:customAction(function(player, sentence, npc)
		local targetName = sentence:getTrimmedText():gsub("^%s*(.-)%s*$", "%1"):lower()
		local target = getPlayer(targetName)

		if target == nil then
			target = getNPC(targetName)
		end
...
if game:propertyEnabled("stendhal.testserver") then
	...
	if game:setZone(zone) then
		...
		if npc ~= nil then
			...
			logger:error("Could not create NPC")
		end
	else
